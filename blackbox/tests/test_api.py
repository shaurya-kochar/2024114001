...
def test_delete_existing_product():
    ...
    post_res = requests.post(f"{BASE_URL}/products", json=payload, headers=headers)
    if post_res.status_code == 201:
        pid = post_res.json().get("id")
        del_res = requests.delete(f"{BASE_URL}/products/{pid}", headers=headers)
...
def test_get_negative_id():
    response = requests.get(f"{BASE_URL}/products/-5", headers=headers)
    assert response.status_code == 400 or response.status_code == 404

def test_get_invalid_string_id():
    response = requests.get(f"{BASE_URL}/products/invalid_id_abc", headers=headers)
    assert response.status_code == 400 or response.status_code == 404
...
def test_delete_already_deleted_product():
    ...
    response = requests.delete(f"{BASE_URL}/products/999998", headers=headers)
    assert response.status_code == 404

def test_put_invalid_string_id():
    response = requests.put(f"{BASE_URL}/products/xyz123", json={"name": "A"}, headers=headers)
    assert response.status_code in [400, 404]

def test_put_negative_id():
    response = requests.put(f"{BASE_URL}/products/-1", json={"name": "A"}, headers=headers)
    assert response.status_code in [400, 404]
...
def test_post_empty_body():
    response = requests.post(f"{BASE_URL}/products", json={}, headers=headers)
    assert response.status_code == 400

def test_put_missing_all_fields():
    response = requests.put(f"{BASE_URL}/products/1", json={}, headers=headers)
    assert response.status_code == 400
...
def test_post_no_json_header():
    ...
    response = requests.post(f"{BASE_URL}/products", data={"name": "Test", "price": 10, "stock": 1}, headers=headers)
